...
class Downline extends Component
{
    ...
    public function render()
    {
        ...
        DB::raw('(select ifnull(contract_price, 0) from user a where a.actived_at is not null and left(a.network, length(concat(user.network, user.id, "ka")))=concat(user.network, user.id, "ka") ) right_turnover'))->where('username', $this->key);
